...
- S98 chip-split runtime gate (5a/5c) — NOT yet walked on device: tap-save-and-read-storage
  (FR UI must still store EN codes — e.g. windDir "E", weather ["Sunny","Fog"], never French
  text) + a legacy 16-point windDir (e.g. NNE) history render. The split is diff-verified
  (writes/sentinel/compares untouched) + throwaway-tested vs the real locale JSONs, but the
  on-device save/read proof is outstanding. Belt-and-braces: 'No Fishing' exclusivity taps.
- Firebase FR verification email — setLanguageCode IS wired before both sends (S98), but the
  email template is customized in the Firebase console and the "email verified" landing page
  is still EN: needs a real FR signup send to confirm. If it stays EN, the open decision is
  revert-template-to-default (Firebase auto-localizes defaults) vs a Cloud Function. Console-
  side; Jonny owns.
- Non-French S98 fix-list items — still OPEN in the screenshot-gate queue (S98 was French-
  only; none touched): PDF-viewer inset, Confirm-Trip-Start double header, DFO-docs-card
  dfoActivated gate.
...
SESSION 98 — FRENCH SIGNUP + HOME + CHIP SPLIT complete (four commits pushed, tip 70b16d1; see session log S98). French NOT marked done — PENDING (Jonny): chip-split runtime gate 5a/5c device walk (FR UI must store EN codes; legacy NNE render); real FR signup to test the customized Firebase verification-email template (landing page still EN — revert-vs-Cloud-Function decision open); proofreader pile now ~113 strings (35 S97 + ~78 S98, handoff timing open); non-French fix-list items (PDF-viewer inset, Confirm-Trip-Start double header, DFO-docs-card dfoActivated gate) still queued for the screenshot gate. S97 pendings (FR checklist runs on Pixel 8) still ride. SESSION 99 — TBD