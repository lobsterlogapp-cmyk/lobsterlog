...
Last updated: July 18, 2026 (Session 103 — HASH BACKFILL + MASTER SHOT LIST (recon + docs only, ZERO code edits, NO-GIT, no network): S102 commit hashes verified post-push and backfilled into the log — fb7d3ee (§17 swap) · 805bb08 (gate+report) · bec6d22 (CLAUDE.md), range 2ce0969..bec6d22; 3 S102-hash PENDING-VERIFICATION markers resolved in place, unrelated PENDING markers (general-policy line, S99 6(d) pill walk) left untouched. Built the master §22 capture shot list in docs/GATE_S103_SHOTLIST.md: placeholder counts §22 EN 16 [ SCREENSHOT ] = 16 expected (PASS) · §22 FR 16 [ CAPTURE D'ÉCRAN ] (parity PASS) · §17 EN/FR 0/0 (parity PASS); 16 figure rows (F01–F16) + 6 TRG transmission-log rows (Grid1-T5 ×3 EN/×3 FR, 222-T7/T8, 233-T3/T4 — capture-scheme IDs, distinct from S58 XML test codes); canonical seed-trip spec APPROVED (MAR region 90, reserved MAR-90 triplet VRN 1004460, LFA 34 + picker-valid grid, 2026-07-06 timestamps, 200 traps, Lobster/250 lbs, Mackerel/Frozen/40 lbs); dual-sim dry-run plan (activate once, toggle language within groups). RULING July 18: **weather OUT of scope for capture** (no §22 figure frames the free Daily Log/history) — weather display bugs incl. "0.0nd" stay on the Aug/Sept pile, unchanged. **DECISION: DfoSetupScreen IS in-frame** (F06 Settings item + F07 setup screen with region picker) → **L5 regions + L5-adjacent JBE strings GATE the FR capture of F06/F07** (FR captions already read « Configuration du JBE du MPO » / « sélecteur de région » but the app shows « Configuration MPO / Clé ELOG » + EN regions); EN F06/F07 fine; **L5 does NOT slide**. All in-app §22 shots use the **dfoelog demo** account (admin "Lots-0-Lobster" not in the capture path); the demo account (dfoelog@lobsterlog.com signup + role + demo vessel profile) is the **blocking prerequisite** and is still an S99 TO-DO. No tsc/jest (no code touched). PRIOR: Session 102 — §17/§22/§25 COMPLIANCE-DOC REFRESH (DOCS session, no generator/logic change): §17 Provider's Instructions swapped v1.1→v1.2 [234.6→234.7, FR NEB/voyage/levés/levée, JBE kept per ruling (b)], §22 User's Guide re-exported v1.0→v1.1 + §25 Prerequisites re-exported [package-only, UNTRACKED], App.tsx §17 require paths repointed, H3 typo «Presrsquisites» fixed; §22/§25 RULED PACKAGE-ONLY on rule text; §22 §12 self-claim fixed. tsc 33/0-new, jest 19/68, sim render walk PASSED. Commits fb7d3ee · 805bb08 · bec6d22 (verified S103). Full record docs/GATE_S102_DOCS.md + docs/S102_EDIT_REPORT.md. Carry-forward queue: L5 regions (glossary-first recon) + L5-adjacent DfoSetupScreen JBE strings (now GATE the §22 FR F06/F07 capture), dfoelog demo account setup (blocks all in-app capture), off-MAR bycatch/pcons 36-vs-2, units-label-vs-pref LBS/kg, LFA→ZPH, weather display bugs (Aug/Sept). Next — SESSION 104 TBD)
...
- **L5 — DfoSetupScreen region labels (EN in FR mode)** — REGIONS (DfoSetupScreen.tsx:30)
  ...
  need MPO-standard FR from the answer key (Golfe / Québec / T.-N.-L. or full forms) — do
  NOT invent on a role-gated setup screen. The only user-facing EN option list left after
  the S101b codebase-wide picker sweep.
