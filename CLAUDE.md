...
Last updated: July 15, 2026 (Session 99 — NON-FRENCH SCREENSHOT-GATE REMAINDER + ROLE-GATED DFO ACCESS; six commits, run by Jonny per NO-GIT. [1] 69b1515 PDF-viewer Close inset: core RN SafeAreaView is iOS-only + Android edge-to-edge → insets-driven padding (S95 pattern) in the App.tsx doc-viewer Modal; VERIFIED Pixel 8, iOS-neutral confirmed. [2] 9984c96 Confirm-Trip-Start "double header": spurious Android StatusBar.currentHeight band on a screen rendered BELOW the persistent app header — padding removed; VERIFIED Pixel; color verdict: flush blue bars read fine, no recolor. [3] 5647ca1 Settings DFO-docs card gated {dfoActivated === true} — Rule 2500 reasoning ON RECORD in docs/GATE_S99_REMAINDER.md §0.3 (dfoActivated=false ⇒ no DFO obligations ⇒ hiding ≠ violation); hidden-state VERIFIED on device; shown-state VERIFIED for the activated dfo-role account. [4] 42e8b07 DEV bypass toggle DELETED from DfoSetupScreen (recon found it never persisted dfoActivated — session-only). [5] 96a5b72 role-gated FREE activation: canActivateDfoFree memo (role 'admin'|'dfo', console-assigned string, migration-free) → DfoSetupScreen skips ONLY the two RevenueCat calls and falls through to the SAME saveCaptainProfile({…, dfoActivated:true}) write; handleRestore + Pro untouched; RESTART GATE PASSED IN FULL (no-purchase activation / persists across force-quit / docs card shown / zero dev chrome / control routes to purchase). [6] pill render gated (canActivateDfoFree || dfoActivated===true) — invite-only DFO surface, no side doors (§5.6 sweep); XML harness RETAINED __DEV__-only by decision. Proofreader tally AUDITED: exactly 113 strings (99 physical rows; 9 trivial FR=EN wind codes → real workload 104). tsc 33/0-new + jest 19/68 every phase. PART-1 FRENCH VERIFICATION RECORDED (device/console-confirmed July 15): (1a) FR save stored windDir "E", weather ["Cloudy","Sunny"] — EN codes in Firestore, invariant held; (1b) legacy 16-point renders NNO, no key paths; (1c) Pas de pêche exclusivity fires, sentinel round-trips; (1d) verification email arrived in FRENCH (customized template honored languageCode — corrects the S98 caution); hosted landing page remains English despite &lang=fr — ACCEPTED as Firebase-side limitation, screenshot on file. **French: DONE.** PENDING VERIFICATION: 6(d) pill visibility walk; P3 admin-role shown state; iOS sweep of P2/P3/6a/6b/6d. TO DO before the submission package: dfoelog@lobsterlog.com signup + role + demo vessel profile. Full phase/gate record: docs/GATE_S99_REMAINDER.md) [1] a282d4c login/signup surface: new common.json login.* section [26 keys], LoginScreen 3 states + useAuth's 7 submit alerts t()-wired, modular setLanguageCode(auth, i18next.language) before BOTH email sends [RNFirebase v23; useDeviceLanguage() throws on native]. [2] ea5f513 Daily Log home: 19 log.* keys, 15 App.tsx labels + 4 useLogForm save alerts, date header en-US → fr-CA/en-CA. [3] 6ab38e1 history card: 8 keys + reused lbsSuffix, 3 date sites → fr-CA/en-CA, DateTimePicker locale prop. [4] 70b16d1 chip code↔label split: stored values stay canonical EN codes [ZERO migration; writes/sentinel/compares diff-proven untouched], new src/utils/chipLabels.ts with defaultValue:code fallback, 16-wind + 9-condition label maps, 4 render sites. tsc 33/0-new + jest 19/68 every commit; EN+FR live-verified on sim off same stored codes. ~78 FR strings → proofreader pile (now ~113 w/ S97's 35), docs/GATE_S98_FRENCH_FIX.md. PENDING: chip gate 5a/5c device walk; FR verification-email template test. French NOT done. Next — TBD; Session 97 summary in the session log)
...
SESSION 99 — complete (six commits; see session log S99 + docs/GATE_S99_REMAINDER.md). French: DONE (Part-1 results recorded). Riding pendings: 6(d) pill visibility walk, P3 admin-role shown state, iOS sweep of P2/P3/6a/6b/6d, proofreader handoff (113 strings), submission-package TO DO (dfoelog@lobsterlog.com signup + role + demo vessel profile). SESSION 100 — TBD