...
Last updated: July 19, 2026 (Session 105 — DFOELOG DEMO ACCOUNT SETUP (account setup + verification; NO code edits, NO-GIT state-changing, NO-DFO-POST). Stood up the dfoelog@lobsterlog.com demo account — the SOLE remaining blocker for ALL in-app §22/TRG capture (S99→S104 carry-forward). Phase 0 recon (role schema §5.1, seed spec §2.3, canActivateDfoFree) → Gate 0 → Phase 1 (Jonny drove the iPhone-17-Pro sim, Claude verified each step from Firestore REST + sim AsyncStorage reads) → Phase 2 → closeout. ACCOUNT uid AHfUKxuIPsOjrGaK0yujk1TpyFP2: role 'dfo' set in Firestore (default) users/{uid}/settings/profile (Firestore read = 'dfo' exact); byte-correct MAR-90 profile — operatorName 'LobsterLog Demo', FIN 100400460 / VRN 104460 / Fishing Licence 104460 (the reserved 6-digit MAR row from Test_values p.1, VRN=LIC per row — FLAG-A ruling, dated correction appended to GATE_S103_SHOTLIST §2.3), LFA 34, 200/Traps, ELOG key from .env, Cloud Backup OFF (Option b); FREE-ACTIVATED via canActivateDfoFree (role 'admin'|'dfo', no purchase → dfoActivated:true), persisted across force-quit; ZERO dev chrome (isAdmin=false → no DEV float, no XML harness). CAUGHT+fixed a FIN/Licence data-entry swap (neither validator catches it — 104460 is a valid FIN per Rule 260). Region ruling Option (b): activate MAR-90 on the capture sim only, backup OFF, TRG XML = the guaranteed four-region evidence (docs/RECON_S105_DEMO_SCOPE.md). A mid-session app-delete (dev-env launch fix) WIPED local AsyncStorage → LIVE-VALIDATED Option (b) (backup OFF ⇒ nothing restored ⇒ self-serve rebuild; auth+role survived in cloud) and confirmed Privacy Notice + Language picker are DEVICE-GLOBAL flags (per-uid consent flag → Aug/Sept pile); rebuilt + re-verified byte-correct. Phase 2: New Boat throwaway (uid G6lxWh…, role 'dfo') DELETED off Firebase (auth + Firestore doc both NOT_FOUND-verified); demo credentials go to DFO out-of-band at submission, never in repo. PROVENANCE CORRECTION (master context): the June 24 2026 meeting record lives in Gmail (Kane's follow-up) + Otter transcript, NOT an S82 gate doc. **Demo account now LIVE → §22/TRG in-app capture FULLY UNBLOCKED (run the S103 dry-run plan).** Full record docs/GATE_S105_DEMO_ACCOUNT.md. NO-GIT: one commit / FOUR files (CLAUDE.md + GATE_S105 + GATE_S103 §2.3 correction + RECON_S105_DEMO_SCOPE), Jonny runs. PRIOR: Session 104 — L5 DfoSetupScreen region labels FR + L5-adjacent JBE strings (display-only, NO-GIT); §22 FR F06/F07 app-side unblocked; docs/GATE_S104_L5_REGIONS.md. Next — SESSION 106 TBD)
...
- Submission-package TO DO (Jonny): dfoelog@lobsterlog.com signup + role assignment + demo
  ...
  persistent. **§22/TRG in-app capture now FULLY UNBLOCKED** — run the S103 dry-run plan.
