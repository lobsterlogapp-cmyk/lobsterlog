...
Last updated: July 18, 2026 (Session 104 — L5 DfoSetupScreen REGION LABELS FR + L5-ADJACENT JBE STRINGS (display-only, NO-GIT, no network / no DFO POST). Glossary-first recon → value/display-only edits across 4 files; stored `subformId` byte-identical, `git diff src/utils/` = 0. **REGIONS** (DfoSetupScreen.tsx) now render via `t(labelKey, { defaultValue: EN pill })` — FR from `MV_DFO_REGION_rel3.csv` DESC_FRE (regId == CODE_ID; corroborated by §17 §2.4 L100): 90 **Maritimes** / 89 **Golfe** / 88 **Québec** / 91 **Terre-Neuve-et-Labrador**; EN pills Maritimes/Gulf/Quebec/Nfld & Lab UNCHANGED (defaultValue fallback). **JBE terminology** (ruling b / S100 D4, answer key = §17 §2.4 + `MV_DFO_REGION`; SWAP-TERM-ONLY per Gate-0 FLAG C — app voice retained): fr `setup.headerTitle` → « Configure ton JBE du MPO », `setup.activateButton` → « Activer le JBE du MPO », `setup.priceLabel` → « LAISSEZ-PASSER DE SAISON JBE DU MPO », orphaned `setup.title` → « Configuration du JBE du MPO » (hygiene); fr `common.profile.elogKey`/`elogKeyLabel` → « Clé JBE » / « CLÉ JBE » (renders on CaptainProfileScreen, L5-adjacent per ruling b). **ALL EN byte-unchanged** (EN keeps "ELOG" per S100 T2). +4 region keys en+fr dfo.json (`setup` 22→26, key-sets symmetric). Purchase-alert strings (DfoSetupScreen Alert.alert, English) LEFT — parked to Aug/Sept paywall i18n. GATES: tsc 33/0-new, jest 19/68, `git diff src/utils/` = 0, babel clean, zero probes, locale key-sets symmetric. **WALK PASSED** (admin Lots-0-Lobster → DEV: Back to Setup; iPhone 17 Pro sim): FR regions render (Terre-Neuve-et-Labrador wraps, no clip) · FR JBE header/price/activate render · EN mode all unchanged · stored `subformId` 88/90 byte-identical after pill taps (display-only, no Activate) · CaptainProfile « CLÉ JBE » FR / « ELOG KEY » EN · routing back to logs list clean. **L5 + L5-adjacent JBE now DONE → the §22 FR F06/F07 capture is UNBLOCKED on the app side; the only remaining blocker for ALL in-app §22/TRG capture is the dfoelog demo account (dfoelog@lobsterlog.com signup + role + demo vessel profile — still the S99 TO-DO).** Full record docs/GATE_S104_L5_REGIONS.md. NO-GIT: two-commit block in the gate doc closeout (C1 code+locale, C2 gate doc + CLAUDE.md; bare subjects, no trailer, staged by exact path). PRIOR: Session 103 — hash backfill (S102 commits fb7d3ee · 805bb08 · bec6d22, range 2ce0969..bec6d22 verified) + master §22 capture shot list (docs/GATE_S103_SHOTLIST.md: §22 EN/FR 16/16 parity, 16 F-rows + 6 TRG rows, seed-trip MAR-90/LFA 34 APPROVED; weather OUT of capture scope; DfoSetupScreen ruled in-frame F06/F07); recon+docs only. Next — SESSION 105 TBD)
...
- Submission-package TO DO (Jonny): dfoelog@lobsterlog.com signup + role assignment + demo
  vessel profile — required before the DFO submission package.
