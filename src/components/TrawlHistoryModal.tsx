...
const ExpandableTrawlCard = ({ item }: { item: any }) => {
    ...
    // Safely parse the Native Timestamp object for display
    let haulTimeString = '--:--';
    if (item.timestamp) {
        const parsedTime = item.timestamp.toDate ? item.timestamp.toDate() : new Date(item.timestamp);
        haulTimeString = parsedTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    }
...
export const TrawlHistoryModal = ({ visible, onClose, currentUser }: any) => {
    ...
    useEffect(() => {
        ...
        const fetchTrawlHistory = async () => {
                    setLoading(true);
                    try {
                        // NEW MODULAR SYNTAX + HAUL DATE FIX
                        const trawlsRef = collection(db, 'users', currentUser.uid, 'trawls');

                        // Note the swap from "dateId" to "haulDate" on the last line!
                        const q = query(
                            trawlsRef,
                            where("status", "==", "history"),
                            where("haulDate", "==", formattedDateId)
                        );
                        ...
                        // Sort by trawl number, then safely handle Native Timestamps for sorting
                        trawls.sort((a, b) => {
        ...
        <Modal animationType="slide" transparent={true} visible={visible} onRequestClose={onClose}>
            <View style={{flex: 1, backgroundColor: '#1E293B', marginTop: 50, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, shadowColor: '#000', shadowOffset: {width: 0, height: -4}, shadowOpacity: 0.2, shadowRadius: 10}}>
                ...
                <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20}}>
                    <Text style={{fontSize: 24, fontWeight: 'bold', color: 'white'}}>Trawl Logbook</Text>
